...
bool t_prune_middle_third_subopt(int n) {
  ...
  psline(A, n3, n3+1, n-n3, 1.0, 0.0);
  psline(A, n3, 2*n3, n-n3-1, 1.0, -1.0);

  psline(Correct, n3, 1, n-1, 1.0, -1.0);
  psline(Correct, n3, 2*n3, n-n3-1, 1.0, -1.0);
...
int main(int argc, char *argv[]) {
  ...
  START("[prune_sorted]: test F: middle n/3 elems are subopt] ");
  printf("    N = %i\n\n ", n);


  TIME_RATIO(t_prune_middle_third_subopt(n), t_prune_middle_third_subopt(2*n), 
     "timed test: middle n/3 elems must be removed", 1, 2.9, 4.0);
