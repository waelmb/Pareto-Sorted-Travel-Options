...
bool t_union_alternatingN_A_dom(int n) {
  ...
  psline(B, n, 1.5, n, 1.0, -1.0);
  psline(Correct, n, 1, n, 1.0, -1.0);
...
int main(int argc, char *argv[]) {
  ...
  START("[union_pareto_sorted : test D: timed - one list completely dominates the other] ");
  printf("    N = %i\n\n ", n);


  TIME_RATIO(t_union_alternatingN_A_dom(n), t_union_alternatingN_A_dom(2*n), 
     "union of two lists (timed); only elems of one list remain ", 1, 2.9, 3.5);
