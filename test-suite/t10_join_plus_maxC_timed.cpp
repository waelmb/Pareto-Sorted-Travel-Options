...
bool t_join_pm_A_always_critical(int n) {
...
//  std::vector<std::pair<double,double>> Correct;
  ...
  psline(A, n, 1, 2*n+1, 1.0, -1.0);
  psline(B, n, 0, n, 1.0, -1.0);
  ...
  bool passed = cmp_vec(A, *result, false);
  delete a;
  delete result;
  return passed;
}
...
int main(int argc, char *argv[]) {
  ...
  START("[join_plus_max]: test C (timed)] ");
  printf("    N = %i\n\n ", n);


  TIME_RATIO(t_join_pm_A_always_critical(n), t_join_pm_A_always_critical(2*n), 
     "timed test: A-list always critical (only 1st elem of B-list used)", 
      1, 2.9, 4.0);
